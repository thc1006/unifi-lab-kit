#!/usr/bin/env python3
"""Fix server11: NOPASSWD, password, SSH port, SSH key, DHCP binding."""
import paramiko
import time
import socket
...
print(f"Connected to server11 at {IP}:{PORT}")
...
if nopasswd == "root":
    ...
    # 7. Need DHCP binding for .111
    print(f"\n7. DHCP binding needed:")
    print(f"  MAC: {MAC}")
    print(f"  Current IP: .220 (DHCP auto)")
    print(f"  Target IP: .111")
    print(f"  Need to add DHCP reservation in UniFi Controller")
