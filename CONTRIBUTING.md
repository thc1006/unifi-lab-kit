# Contributing to unifi-lab-kit

Thanks for considering a contribution. This toolkit is deliberately small and opinionated — please keep it that way.

## Ground rules

1. **No secrets in code or committed files.** Everything that looks like a password, a public IP, a MAC address, or an SSH public key must come from `.env` (via `src/unifi_lab_kit/config.py`) or from `inventory/hosts.yml` (local, gitignored). If you find a literal credential anywhere in `src/` or `docs/`, that's a bug — fix it.
